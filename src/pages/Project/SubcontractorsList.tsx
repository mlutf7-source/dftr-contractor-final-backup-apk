import {
  Link,
  useOutletContext,
} from 'react-router-dom';

import {
  useEffect,
  useState,
} from 'react';

import type {
  Project,
  Subcontractor,
} from '../../models/types';

import {
  saveProject,
} from '../../database/database';

import {
  now,
  uid,
} from '../../utils/format';

import AppCard from '../../components/cards/AppCard';
import SuggestInput from '../../components/forms/SuggestInput';
import TextAreaInput from '../../components/forms/TextAreaInput';

type Ctx = {
  project: Project;
  setProject: (project: Project) => void;
};

export default function SubcontractorsList() {
  const {
    project,
    setProject,
  } = useOutletContext<Ctx>();

  const [p, setP] =
    useState<Project>(project);

  useEffect(() => {
    setP(project);
  }, [project]);

  const applyProjectUpdate = (
    updated: Project
  ) => {
    const saved =
      saveProject(updated);

    setP(saved);
    setProject(saved);
  };

  const [showForm, setShowForm] =
    useState(false);

  const [editingId, setEditingId] =
    useState('');

  const [form, setForm] =
    useState({
      name: '',
      trade: '',
      phone: '',
      notes: '',
    });

  const reset = () => {
    setEditingId('');

    setForm({
      name: '',
      trade: '',
      phone: '',
      notes: '',
    });

    setShowForm(false);
  };

  const save = () => {
    if (!form.name.trim()) {
      alert('أدخل اسم مقاول الباطن');
      return;
    }

    const date = now();

    let subcontractors: Subcontractor[];

    if (editingId) {
      subcontractors =
        p.subcontractors.map((s) =>
          s.id === editingId
            ? {
                ...s,
                name: form.name,
                trade: form.trade,
                phone: form.phone,
                notes: form.notes,
                updatedAt: date,
              }
            : s
        );
    } else {
      const row: Subcontractor = {
        id: uid(),
        name: form.name,
        trade: form.trade,
        phone: form.phone,
        notes: form.notes,
        agreementAmountYER: 0,
        originalAgreementAmount: 0,
        currency: 'YER',
        exchangeRate: 1,
        createdAt: date,
        updatedAt: date,
      };

      subcontractors = [
        row,
        ...p.subcontractors,
      ];
    }

    const updated: Project = {
      ...p,
      subcontractors,
      updatedAt: date,
    };

    applyProjectUpdate(updated);
    reset();
  };

  const edit = (s: Subcontractor) => {
    setEditingId(s.id);

    setForm({
      name: s.name,
      trade: s.trade,
      phone: s.phone,
      notes: s.notes,
    });

    setShowForm(true);

    window.scrollTo({
      top: 0,
      behavior: 'smooth',
    });
  };

  const remove = (id: string) => {
    if (
      !confirm(
        'هل تريد حذف مقاول الباطن؟ سيتم حذف بياناته من هذا المشروع.'
      )
    ) {
      return;
    }

    const updated: Project = {
      ...p,
      subcontractors:
        p.subcontractors.filter(
          (s) => s.id !== id
        ),
      updatedAt: now(),
    };

    applyProjectUpdate(updated);
  };

  return (
    <div>
      {!showForm && (
        <button
          className="btn primary"
          style={{ marginBottom: 14 }}
          onClick={() => {
            reset();
            setShowForm(true);
          }}
        >
          + إضافة مقاول باطن
        </button>
      )}

      {showForm && (
        <AppCard
          title={
            editingId
              ? '✏️ تعديل مقاول باطن'
              : '🏗️ إضافة مقاول باطن'
          }
        >
          <SuggestInput
            label="اسم المقاول"
            value={form.name}
            typeKey="subcontractorNames"
            onChange={(v) =>
              setForm({
                ...form,
                name: v,
              })
            }
          />

          <SuggestInput
            label="التخصص"
            value={form.trade}
            typeKey="trades"
            onChange={(v) =>
              setForm({
                ...form,
                trade: v,
              })
            }
          />

          <label className="field">
            <span>الهاتف</span>

            <input
              value={form.phone}
              inputMode="tel"
              onChange={(e) =>
                setForm({
                  ...form,
                  phone: e.target.value,
                })
              }
            />
          </label>

          <TextAreaInput
            label="ملاحظات"
            value={form.notes}
            onChange={(v) =>
              setForm({
                ...form,
                notes: v,
              })
            }
          />

          <div className="row">
            <button
              className="btn primary"
              onClick={save}
            >
              {editingId
                ? 'حفظ التعديل'
                : 'إضافة'}
            </button>

            <button
              className="btn gray"
              onClick={reset}
            >
              إلغاء
            </button>
          </div>
        </AppCard>
      )}

      <AppCard title="قائمة مقاولي الباطن">
        {!p.subcontractors.length && (
          <div className="empty-state">
            لا يوجد مقاولون حتى الآن
          </div>
        )}

        <div className="grid">
          {p.subcontractors.map((s) => (
            <div
              className="card"
              key={s.id}
            >
              <h3>{s.name}</h3>

              <p>
                <b>التخصص:</b>{' '}
                {s.trade || '-'}
              </p>

              <p>
                <b>الهاتف:</b>{' '}
                {s.phone || '-'}
              </p>

              {s.notes && (
                <p>{s.notes}</p>
              )}

              <div className="row">
                <Link
                  className="btn primary"
                  to={`/project/${p.id}/subcontractor/${s.id}`}
                >
                  فتح الحساب
                </Link>

                <button
                  className="btn gray"
                  onClick={() => edit(s)}
                >
                  تعديل
                </button>

                <button
                  className="btn danger"
                  onClick={() => remove(s.id)}
                >
                  حذف
                </button>
              </div>
            </div>
          ))}
        </div>
      </AppCard>
    </div>
  );
          }
