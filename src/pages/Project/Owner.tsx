import {
  useEffect,
  useState,
} from 'react';

import { useOutletContext } from 'react-router-dom';

import type {
  CurrencyCode,
  Project,
} from '../../models/types';

import {
  addOwnerItem,
  addOwnerMaterial,
  addOwnerOtherCost,
  addOwnerPayment,
} from '../../database/transactions';

import {
  getOwnerSummary,
} from '../../database/calculations';

import {
  fromYER,
} from '../../utils/currency';

import {
  today,
} from '../../utils/format';

import CurrencySelector from '../../components/common/CurrencySelector';
import CollapseCard from '../../components/cards/CollapseCard';
import SummaryCards from '../../components/cards/SummaryCards';

import DateInput from '../../components/forms/DateInput';
import MoneyInput from '../../components/forms/MoneyInput';
import NumberInput from '../../components/forms/NumberInput';
import SuggestInput from '../../components/forms/SuggestInput';
import TextAreaInput from '../../components/forms/TextAreaInput';

import AccountTable from '../../components/reports/AccountTable';
import ReportHeader from '../../components/reports/ReportHeader';
import ReportTotals from '../../components/reports/ReportTotals';

import { printPage } from '../../utils/print';
import { sharePdfFromElement } from '../../utils/pdfShare';

type Ctx = {
  project: Project;
  setProject: (project: Project) => void;
};

export default function Owner() {
  const {
    project,
    setProject,
  } = useOutletContext<Ctx>();

  const [p, setP] =
    useState(project);

  useEffect(() => {
    setP(project);
  }, [project]);

  const applyProjectUpdate = (
    updated: Project
  ) => {
    setP(updated);
    setProject(updated);
  };

  const [currency, setCurrency] =
    useState<CurrencyCode>('YER');

  const [rate, setRate] =
    useState(1);

  const [payment, setPayment] =
    useState({
      amount: 0,
      currency: 'YER' as CurrencyCode,
      date: today(),
      notes: '',
    });

  const [item, setItem] =
    useState({
      title: '',
      quantity: '',
      unit: '',
      unitPrice: 0,
      currency: 'YER' as CurrencyCode,
      notes: '',
    });

  const [material, setMaterial] =
    useState({
      title: '',
      quantity: '',
      unit: '',
      unitPrice: 0,
      currency: 'YER' as CurrencyCode,
      supplier: '',
      notes: '',
    });

  const [cost, setCost] =
    useState({
      title: '',
      amount: 0,
      currency: 'YER' as CurrencyCode,
      notes: '',
    });

  const summary =
    getOwnerSummary(p);

  const convert = (v: number) =>
    fromYER(
      v,
      currency,
      {
        SAR:
          currency === 'SAR'
            ? rate
            : p.exchangeRates.SAR,

        USD:
          currency === 'USD'
            ? rate
            : p.exchangeRates.USD,
      }
    );

  const balanceValue =
    Math.abs(
      convert(summary.balance)
    );

  const items =
    p.transactions.filter(
      t => t.type === 'owner_item'
    );

  const materials =
    p.transactions.filter(
      t => t.type === 'owner_material'
    );

  const otherCosts =
    p.transactions.filter(
      t => t.type === 'owner_other_cost'
    );

  const payments =
    p.transactions.filter(
      t => t.type === 'owner_payment'
    );

  const savePayment = () => {
    if (!payment.amount) return;

    const updated =
      addOwnerPayment(p, {
        amount: payment.amount,
        currency: payment.currency,
        date: payment.date,
        notes: payment.notes,
      });

    applyProjectUpdate(updated);

    setPayment({
      amount: 0,
      currency: 'YER',
      date: today(),
      notes: '',
    });
  };

  const saveItem = () => {
    const qty =
      Number(item.quantity) || 0;

    if (
      !item.title ||
      !qty ||
      !item.unitPrice
    ) {
      return;
    }

    const updated =
      addOwnerItem(p, {
        title: item.title,
        quantity: qty,
        unit: item.unit,
        unitPrice: item.unitPrice,
        currency: item.currency,
        date: today(),
        notes: item.notes,
      });

    applyProjectUpdate(updated);

    setItem({
      title: '',
      quantity: '',
      unit: '',
      unitPrice: 0,
      currency: 'YER',
      notes: '',
    });
  };

  const saveMaterial = () => {
    const qty =
      Number(material.quantity) || 0;

    if (
      !material.title ||
      !qty ||
      !material.unitPrice
    ) {
      return;
    }

    const updated =
      addOwnerMaterial(p, {
        title: material.title,
        quantity: qty,
        unit: material.unit,
        unitPrice: material.unitPrice,
        currency: material.currency,
        date: today(),
        notes: material.supplier
          ? `${material.supplier} - ${material.notes}`
          : material.notes,
      });

    applyProjectUpdate(updated);

    setMaterial({
      title: '',
      quantity: '',
      unit: '',
      unitPrice: 0,
      currency: 'YER',
      supplier: '',
      notes: '',
    });
  };

  const saveCost = () => {
    if (!cost.title || !cost.amount) {
      return;
    }

    const updated =
      addOwnerOtherCost(p, {
        title: cost.title,
        amount: cost.amount,
        currency: cost.currency,
        date: today(),
        notes: cost.notes,
      });

    applyProjectUpdate(updated);

    setCost({
      title: '',
      amount: 0,
      currency: 'YER',
      notes: '',
    });
  };

  return (
    <div>
      <div className="section-title-box">
        <h2>👤 حساب المالك</h2>
      </div>

      <CurrencySelector
        currency={currency}
        rates={p.exchangeRates}
        rate={rate}
        onCurrency={setCurrency}
        onRate={setRate}
      />

      <SummaryCards
        due={convert(summary.due)}
        paid={convert(summary.paid)}
        balance={balanceValue}
        balanceStatus={summary.status}
        balanceColor={summary.color}
        currency={currency}
        dueTitle="الذي عليه"
        paidTitle="الذي له"
        balanceTitle="الرصيد"
      />

      <CollapseCard title="استلام دفعة من المالك">
        <MoneyInput
          label="المبلغ"
          amount={payment.amount}
          currency={payment.currency}
          onAmount={(v) =>
            setPayment({
              ...payment,
              amount: v,
            })
          }
          onCurrency={(v) =>
            setPayment({
              ...payment,
              currency: v,
            })
          }
        />

        <DateInput
          label="التاريخ"
          value={payment.date}
          onChange={(v) =>
            setPayment({
              ...payment,
              date: v,
            })
          }
        />

        <TextAreaInput
          label="ملاحظات"
          value={payment.notes}
          onChange={(v) =>
            setPayment({
              ...payment,
              notes: v,
            })
          }
        />

        <button
          className="btn primary"
          onClick={savePayment}
        >
          حفظ الدفعة
        </button>
      </CollapseCard>

      <CollapseCard title="إضافة بند">
        <SuggestInput
          label="اسم البند"
          value={item.title}
          typeKey="itemNames"
          onChange={(v) =>
            setItem({
              ...item,
              title: v,
            })
          }
        />

        <div className="form-row-2">
          <NumberInput
            label="الكمية"
            value={item.quantity}
            onChange={(e) =>
              setItem({
                ...item,
                quantity: e.target.value,
              })
            }
          />

          <SuggestInput
            label="الوحدة"
            value={item.unit}
            typeKey="units"
            onChange={(v) =>
              setItem({
                ...item,
                unit: v,
              })
            }
          />
        </div>

        <MoneyInput
          label="سعر الوحدة"
          amount={item.unitPrice}
          currency={item.currency}
          onAmount={(v) =>
            setItem({
              ...item,
              unitPrice: v,
            })
          }
          onCurrency={(v) =>
            setItem({
              ...item,
              currency: v,
            })
          }
        />

        <TextAreaInput
          label="ملاحظات"
          value={item.notes}
          onChange={(v) =>
            setItem({
              ...item,
              notes: v,
            })
          }
        />

        <button
          className="btn primary"
          onClick={saveItem}
        >
          حفظ البند
        </button>
      </CollapseCard>

      <CollapseCard title="إضافة مادة">
        <SuggestInput
          label="اسم المادة"
          value={material.title}
          typeKey="materialNames"
          onChange={(v) =>
            setMaterial({
              ...material,
              title: v,
            })
          }
        />

        <div className="form-row-2">
          <NumberInput
            label="الكمية"
            value={material.quantity}
            onChange={(e) =>
              setMaterial({
                ...material,
                quantity: e.target.value,
              })
            }
          />

          <SuggestInput
            label="الوحدة"
            value={material.unit}
            typeKey="units"
            onChange={(v) =>
              setMaterial({
                ...material,
                unit: v,
              })
            }
          />
        </div>

        <MoneyInput
          label="سعر الوحدة"
          amount={material.unitPrice}
          currency={material.currency}
          onAmount={(v) =>
            setMaterial({
              ...material,
              unitPrice: v,
            })
          }
          onCurrency={(v) =>
            setMaterial({
              ...material,
              currency: v,
            })
          }
        />

        <SuggestInput
          label="المورد"
          value={material.supplier}
          typeKey="supplierNames"
          onChange={(v) =>
            setMaterial({
              ...material,
              supplier: v,
            })
          }
        />

        <TextAreaInput
          label="ملاحظات"
          value={material.notes}
          onChange={(v) =>
            setMaterial({
              ...material,
              notes: v,
            })
          }
        />

        <button
          className="btn primary"
          onClick={saveMaterial}
        >
          حفظ المادة
        </button>
      </CollapseCard>

      <CollapseCard title="إضافة تكلفة أخرى">
        <SuggestInput
          label="البيان"
          value={cost.title}
          typeKey="otherCostNames"
          onChange={(v) =>
            setCost({
              ...cost,
              title: v,
            })
          }
        />

        <MoneyInput
          label="المبلغ"
          amount={cost.amount}
          currency={cost.currency}
          onAmount={(v) =>
            setCost({
              ...cost,
              amount: v,
            })
          }
          onCurrency={(v) =>
            setCost({
              ...cost,
              currency: v,
            })
          }
        />

        <TextAreaInput
          label="ملاحظات"
          value={cost.notes}
          onChange={(v) =>
            setCost({
              ...cost,
              notes: v,
            })
          }
        />

        <button
          className="btn primary"
          onClick={saveCost}
        >
          حفظ التكلفة
        </button>
      </CollapseCard>

      <div
        id="print-report"
        className="card print-area"
      >
        <ReportHeader
          title="كشف حساب المالك"
          project={p.name}
          owner={p.ownerName}
          currency={currency}
        />

        <AccountTable
          currency={currency}
          convert={convert}
          hideQuantityColumns={false}
          showNotesColumn={true}
          groups={[
            {
              title: 'البنود',
              rows: items,
            },
            {
              title: 'المواد',
              rows: materials,
            },
            {
              title: 'التكاليف الأخرى',
              rows: otherCosts,
              hideQuantityColumns: true,
            },
            {
              title: 'دفعات المالك',
              rows: payments,
              hideQuantityColumns: true,
            },
          ]}
        />

        <ReportTotals
          due={convert(summary.due)}
          paid={convert(summary.paid)}
          balance={balanceValue}
          balanceStatus={summary.status}
          balanceColor={summary.color}
          currency={currency}
        />

        <div className="row no-print">
          <button
            className="btn primary"
            onClick={() =>
              printPage('print-report')
            }
          >
            طباعة
          </button>

          <button
            className="btn gray"
            onClick={() =>
              sharePdfFromElement(
                'print-report',
                'كشف حساب المالك'
              )
            }
          >
            مشاركة PDF
          </button>
        </div>
      </div>
    </div>
  );
}
